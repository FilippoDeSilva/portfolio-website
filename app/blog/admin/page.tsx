...
export default function BlogAdmin() {
  ...
  const [coverUploadStatus, setCoverUploadStatus] = useState<string | null>(
    null
  );
    ...
    try {
      ...
      if (form.cover_image) {
        try {
          ...
          console.warn(
            "[Cover Upload] could not delete previous cover (continuing):",
            delErr
          );
        }
      }
      ...
      if (data) {
        const url = supabase.storage.from("blog-attachments").getPublicUrl(path)
          .data.publicUrl;
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (data) {
          ...
          if (file.type.startsWith("audio/")) {
            try {
              setAttachmentUploadStatus(
                `Extracting thumbnail for: ${file.name}`
              );
              const thumbnailResponse = await fetch(
                "/api/extract-music-thumbnail",
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
              ...
              <Card className="w-full max-w-2xl mx-auto bg-gradient-to-br from-background via-muted/60 to-background/80 shadow-xl border-0 rounded-2xl">
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="mb-2 text-base">
                    Share your latest thoughts, stories, or updates. Make it
                    ...
                    <div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-1">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setAttachmentUrlInput(e.target.value)
                          }
                        />
                        <button
                          type="button"
                        ...
                        <AttachmentsGrid
                          ...
                          onPreview={(att: {
                            ...
                            if (att?.type?.startsWith?.("image")) {
                              ...
                              console.log(
                                "[AUDIO PREVIEW DEBUG] Audio attachment:",
                                att
                              );
                              ...
                              console.log(
                                "[AUDIO PREVIEW DEBUG] Direct thumbnail from attachment:",
                                thumb
                              );
                              ...
                              if (!thumb) {
                                console.log(
                                  "[AUDIO PREVIEW DEBUG] No direct thumbnail, searching for matching image..."
                                );
                                ...
                                console.log(
                                  "[AUDIO PREVIEW DEBUG] Audio base name:",
                                  base
                                );
                                const img = (form.attachments || []).find(
                                  (x: any) => {
                                    const isImage =
                                    ...
                                    console.log(
                                      "[AUDIO PREVIEW DEBUG] Checking attachment:",
                                      {
                                        name: x.name,
                                        type: x.type,
                                        isImage,
                                ...
                                thumb = img?.url;
                                console.log(
                                  "[AUDIO PREVIEW DEBUG] Found matching image:",
                                  img?.url
                                );
                              }
                              ...
                              console.log(
                                "[AUDIO PREVIEW DEBUG] Final thumbnail to pass:",
                                thumb
                              );
                              setLightbox({
                                open: true,
